...
# Configured from args in main(); lets gs:// paths stream through a bounded local
# LRU cache instead of requiring the whole dataset to fit on local disk.
_GCS_CFG = {"cache_dir": "/workspace/gcs_audio_cache", "max_gb": 80.0, "key_file": "/workspace/google.json"}
_GCS_CACHE = None
...
def _gcs_cache():
    ...
    if _GCS_CACHE is None:
        _GCS_CACHE = gcs_cache.get_cache(_GCS_CFG["cache_dir"], _GCS_CFG["max_gb"], _GCS_CFG["key_file"])
    return _GCS_CACHE
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--min_audio_seconds", type=float, default=0.5)
    ...
    p.add_argument("--gcs_cache_dir", default="/workspace/gcs_audio_cache",
                   help="local disk cache dir for gs:// audio streamed during training.")
    p.add_argument("--gcs_cache_max_gb", type=float, default=80.0,
                   help="cap on the gs:// audio cache size; oldest-accessed files are evicted above it.")
    p.add_argument("--gcs_key_file", default="/workspace/google.json",
                   help="service account JSON for GCS access (used only for gs:// paths).")
    return p.parse_args()
...
def decode_audio(a: Any) -> np.ndarray:
    if isinstance(a, dict) and a.get("array") is not None:
        ...
        if gcs_cache.is_gcs_uri(raw):
            raw = _gcs_cache().local_path(raw)  # download-on-first-access, then cached locally
...
class CohereCollator:
    ...
    def _encode_batch(self, arrays: list[np.ndarray], texts: list[str]) -> dict[str, torch.Tensor]:
    ...
    def _repair_mismatch(
        ...
        # Rare long-clip/tokenization edge case: processor can return inconsistent
        # batch sizes for decoder inputs vs labels. Keep only samples that encode
        # cleanly one-by-one, then rebuild a consistent batch.
        valid_arrays: list[np.ndarray] = []
        valid_texts: list[str] = []
        for arr, txt in zip(arrays, texts):
            try:
                one = self._encode_batch([arr], [txt])
            except Exception:
                continue
            p = one.get("decoder_input_ids")
            t = one.get("labels")
        ...
        if not valid_arrays:
            raise RuntimeError(
                "CohereCollator could not build a valid batch after filtering "
                "mismatched processor outputs."
            )

        enc = self._encode_batch(valid_arrays, valid_texts)
        prompt = enc["decoder_input_ids"]
        trans = enc["labels"]

        # Final guard: enforce equal batch size across all decoder tensors.
        if prompt.shape[0] != trans.shape[0]:
            n = min(prompt.shape[0], trans.shape[0])
            prompt = prompt[:n]
            trans = trans[:n]
            enc["input_features"] = enc["input_features"][:n]
            if "attention_mask" in enc:
                enc["attention_mask"] = enc["attention_mask"][:n]

        return enc, prompt, trans

    def __call__(self, batch: list[dict]) -> dict[str, torch.Tensor]:
        arrays = [decode_audio(ex["audio"]) for ex in batch]
        texts = [clean_text(ex["text"]) for ex in batch]
        enc = self._encode_batch(arrays, texts)
        ...
        if prompt.shape[0] != trans.shape[0] or prompt.shape[0] != len(arrays):
            enc, prompt, trans = self._repair_mismatch(arrays, texts, prompt, trans)

        # Processor omits EOS from labels; without it the model never learns to
        # stop and repeats at inference. Append one pad slot, then overwrite the
        # first pad in each row with EOS.
        trans = torch.cat([trans, trans.new_full((trans.shape[0], 1), pad)], dim=1)
        for i in range(trans.shape[0]):
            first_pad = (trans[i] == pad).nonzero(as_tuple=True)[0]
            trans[i, first_pad[0]] = eos
        # causal-LM loss: feed [prompt, transcript+EOS] to decoder, supervise only transcript+EOS.
...
def load_split(name, args):
    ...
    def keep(text, duration):
        ...
        return bool(t) and len(t) <= args.max_text_chars and (
            duration is None or args.min_audio_seconds <= duration <= args.max_audio_seconds)

    return ds.filter(keep, input_columns=["text", "duration"], num_proc=args.num_workers)


def load_csv_split(csv_path: str, args) -> Any:
    """Load a pipe-delimited CSV for CohereCollator: wav_path|transcription, or
    wav_path|transcription|duration_seconds (see precompute_gcs_durations.py).

    For gs:// paths we deliberately skip the existence + sf.info() probe (that
    would be one network round-trip per row -- far too slow/costly for a huge
    GCS-backed dataset). Provide the 3rd duration column (computed offline) for
    accurate min/max-length filtering of gs:// rows; without it they're kept
    unfiltered by length. Actual audio bytes are only fetched lazily, per-sample,
    during training via the bounded local cache (see gcs_cache.py).
    """
    ...
    with open(csv_path, "r", encoding="utf-8") as fh:
        for line in fh:
            ...
            dur_field = parts[2].strip() if len(parts) >= 3 else ""
            t = clean_text(text)
            if not t or len(t) > args.max_text_chars:
                continue
            if gcs_cache.is_gcs_uri(wav_path):
                if dur_field:
                    dur = float(dur_field)
                    if dur < args.min_audio_seconds or dur > args.max_audio_seconds:
                        continue
                else:
                    n_gcs_no_duration += 1
            else:
    ...
    if n_gcs_no_duration:
        print(f"WARNING: {n_gcs_no_duration} gs:// rows had no precomputed duration column; "
              f"length filtering skipped for them. Run precompute_gcs_durations.py for accurate filtering.")
...
def main() -> None:
    ...
    _GCS_CFG.update(cache_dir=args.gcs_cache_dir, max_gb=args.gcs_cache_max_gb, key_file=args.gcs_key_file)
